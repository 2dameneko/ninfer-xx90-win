...
| `--kv-dtype bf16\|int8\|fp8\|nvfp4\|k8v4\|rk8v4\|rk4v4\|rk4v4-e8\|rk2v4-e8` | KV-cache storage | `bf16` |
...
`--kv-dtype` selects runtime KV storage. The `fp8`, `nvfp4`, and `k8v4` storage modes require an
RTX 5090 and are rejected at startup on older cards; the remaining modes run on every supported
card. The prepared prompt must fit
