...
//
// Rotated/packed RK/E8 append variants (port of the RTX 4090 fork kernels). One warp handles one
// (token, kv-head, 64-group) unit; K/V receive the H64 pair rotation, packed planes store two
// signed 4-bit codes per byte, and the E8 variants encode keys through the lattice/root codecs.
// The plain INT8 kernels above keep the upstream D256 rotation and are not affected.
...
template <typename Geometry, bool PackedV, bool RotateK, bool RotateV, bool PackedK,
          bool E8Lattice = false, bool E8Root = false, typename Metadata>
__launch_bounds__(256) __global__ void kv_cache_append_full_rk_page_kernel(
