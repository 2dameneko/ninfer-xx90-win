...
#if defined(__CUDA_ARCH__) && defined(NINFER_ENABLE_E8_VADD4)
...
#else
    const uint32_t dir_lo = __vadd4(static_cast<uint32_t>(w_root), static_cast<uint32_t>(w_axis));
    const uint32_t dir_hi = __vadd4(static_cast<uint32_t>(w_root >> 32),
                                    static_cast<uint32_t>(w_axis >> 32));
#endif
