#include "core/saturating_arithmetic.h"
...
std::uint64_t saturating_product(std::uint64_t left, std::uint64_t right) noexcept {
    return core::saturating_multiply(left, right);
}

std::uint64_t q32_product_ns(std::uint64_t coefficient, std::uint64_t units) noexcept {
    if (coefficient == 0 || units == 0) { return 0; }
    const std::uint64_t high = core::multiply_high(coefficient, units);
    const std::uint64_t low  = coefficient * units;
    // A product at or above 2^96 cannot survive the Q32 rescaling inside 64 bits.
    if (high >= kContextCostQ32One) { return std::numeric_limits<std::uint64_t>::max(); }
    // 2^64 is a multiple of 2^32, so rounding the low half up is independent of the high half.
    const std::uint64_t low_ceiling =
        (low >> 32U) + ((low & (kContextCostQ32One - 1U)) != 0 ? 1ULL : 0ULL);
    return (high << 32U) + low_ceiling;
}
